...
async function openClientLoanReadView(id, historyOnly = false) {
  ...
  const status = loan.record_status || (loan.returned_at ? 'encerrado' : 'ativo');
  const statusLabel = status === 'ativo' ? 'Instalado no cliente' : status === 'encerrado' ? 'Devolvido / encerrado' : status === 'pendente_analise' ? 'Pendente de análise' : status;
  const statusClass = status === 'ativo' ? 'saida' : status === 'encerrado' ? 'entrada' : 'low';
  const equipmentName = itemProduct?.name || loan.equipment_name_original || 'Equipamento não informado';
  const equipmentModel = itemProduct?.model || loan.model_original || loan.brand_original || '—';
  $('#client-loan-read-title').textContent = historyOnly ? 'Histórico do comodato' : 'Detalhes do comodato';
  $('#client-loan-read-equipment').textContent = equipmentName;
  $('#client-loan-read-model').textContent = equipmentModel;
  $('#client-loan-read-status').textContent = statusLabel;
  $('#client-loan-read-status').className = `badge ${statusClass}`;
  $('#client-loan-read-details').innerHTML = `<section><h3>Detalhes do equipamento</h3><dl><div><dt>Modelo</dt><dd>${esc(equipmentModel)}</dd></div><div><dt>Patrimônio</dt><dd>${esc(serialItem?.asset_tag || loan.asset_tag_original || '—')}</dd></div><div><dt>MAC</dt><dd>${esc(serialItem?.mac_address || loan.mac_original || '—')}</dd></div><div><dt>Serial</dt><dd>${esc(serialItem?.serial_number || loan.serial_original || '—')}</dd></div></dl></section><section><h3>Vínculo atual</h3><dl><div class="client-loan-customer-card"><dt>Cliente atual</dt><dd>${esc(loan.customer_name || '—')}</dd></div><div><dt>Localização</dt><dd>${esc(loan.city || loan.location_original || '—')}</dd></div><div><dt>Instalação</dt><dd>${date(loan.installed_at || loan.issued_at)}</dd></div><div><dt>Status</dt><dd>${esc(statusLabel)}</dd></div></dl></section>`;
  const returnButton = $('#client-loan-read-return');
  returnButton.hidden = !(status === 'ativo' && loan.serial_item_id && !loan.returned_at);
  returnButton.onclick = () => { $('#client-loan-read-dialog').close(); openClientLoanReturn(id); };
...
async function finishInventory() {
  try {
    ...
    const differences = counts.map(inventoryDifference);
    const divergenceCount = differences.filter(value => value !== 0).length;
    const correctCount = counts.length - divergenceCount;
    const unitsToAdd = differences.reduce((total, value) => total + Math.max(value, 0), 0);
    const unitsToRemove = differences.reduce((total, value) => total + Math.max(-value, 0), 0);
    const hasDifferences = divergenceCount > 0;
    if (!hasDifferences && (unitsToAdd !== 0 || unitsToRemove !== 0)) throw new Error('Falha de consistência na prévia. Nenhum estoque foi alterado.');
    ...
    const preview = `Prévia da finalização:\n\n${correctCount} item(ns) correto(s)\n${divergenceCount} divergência(s)\n+${quantity(unitsToAdd)} unidade(s) a adicionar\n-${quantity(unitsToRemove)} unidade(s) a retirar\n\nFinalizar a conferência?`;
